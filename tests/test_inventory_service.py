...
def test_record_stock_count_creates_adjustment_for_variance():
    ...
    with app.app_context():
        ...
        product = Product(sku="INV-001", name="Inventory Test", default_selling_price=10.0)
...
def test_transfer_stock_creates_movement_between_warehouses():
    ...
    with app.app_context():
        ...
        product = Product(sku="INV-002", name="Transfer Test", default_selling_price=10.0)
        product.quantity_in_stock = 10
        db.session.add(product)
        db.session.flush()
        ...
        source_wh = Warehouse(name="Source Warehouse", is_active=True)
        target_wh = Warehouse(name="Target Warehouse", is_active=True)
